...
describe('Transaction Items', () => {
	describe('Items field structure', () => {
		...
		it('should route items to body', () => {
			const items = findProperty('items');
			expect(items!.routing!.send!.type).toBe('body');
			expect(items!.routing!.send!.property).toBe('items');
		});
	});
	...
	describe('Item fields', () => {
		const getItemValues = () => {
			const items = findProperty('items');
			return (items!.options as INodePropertyCollection[])[0].values;
		};

		it('should have title field', () => {
			const values = getItemValues();
			const title = values.find((v) => v.name === 'title');
		...
		it('should have unitPrice in cents', () => {
			const values = getItemValues();
			const price = values.find((v) => v.name === 'unitPrice');
		...
		it('should have quantity field defaulting to 1', () => {
			const values = getItemValues();
			const qty = values.find((v) => v.name === 'quantity');
		...
		it('should have tangible boolean defaulting to true', () => {
			const values = getItemValues();
			const tangible = values.find((v) => v.name === 'tangible');
		...
		it('should have additionalFields with externalRef', () => {
			const values = getItemValues();
			const additional = values.find((v) => v.name === 'additionalFields');
			expect(additional).toBeDefined();
			const extRef = (additional!.options as INodeProperties[]).find((o) => o.name === 'externalRef');
	...
	describe('Shipping fields', () => {
		...
		it('should have shipping fee in cents', () => {
			const shipping = findProperty('shippingFields');
			const fee = (shipping!.options as INodeProperties[]).find((o) => o.name === 'fee');
		...
		it('should route shipping fields with shipping. prefix', () => {
			const shipping = findProperty('shippingFields');
			(shipping!.options as INodeProperties[]).forEach((opt) => {
		...
		it('should default shipping country to br', () => {
			const shipping = findProperty('shippingFields');
			const country = (shipping!.options as INodeProperties[]).find((o) => o.name === 'country');
	...
	describe('Transaction additional fields', () => {
		it('should have boleto expiration in days', () => {
			...
			const boleto = (additional!.options as INodeProperties[]).find(
				(o) => o.name === 'boletoExpiresInDays',
			);
		...
		it('should have PIX expiration in minutes', () => {
			const additional = findProperty('additionalFields');
			const pix = (additional!.options as INodeProperties[]).find(
				(o) => o.name === 'pixExpiresInMinutes',
			);
		...
		it('should have traceable toggle', () => {
			const additional = findProperty('additionalFields');
			const traceable = (additional!.options as INodeProperties[]).find(
				(o) => o.name === 'traceable',
			);
		...
		it('should have metadata field', () => {
			const additional = findProperty('additionalFields');
			const metadata = (additional!.options as INodeProperties[]).find(
				(o) => o.name === 'metadata',
			);
		...
		it('should have customer IP field', () => {
			const additional = findProperty('additionalFields');
			const ip = (additional!.options as INodeProperties[]).find((o) => o.name === 'ip');
	...
	describe('Filters for getMany', () => {
		...
		it('should support all expected filter types', () => {
			const filters = findProperty('filters');
			const filterNames = (filters!.options as INodeProperties[]).map((o) => o.name);
		...
		it('should route all filters as query parameters', () => {
			const filters = findProperty('filters');
			(filters!.options as INodeProperties[]).forEach((opt) => {
