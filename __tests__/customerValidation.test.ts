...
describe('Customer & Document Validation', () => {
	...
	describe('Document type in transaction', () => {
		it('should support CPF and CNPJ document types', () => {
			...
			const docField = (customerFields!.options as INodeProperties[]).find(
				(o) => o.name === 'document',
			);
			expect(docField).toBeDefined();
			const docValues = (docField!.options as INodePropertyCollection[])[0].values;
			const typeField = docValues.find((v) => v.name === 'type');
			const typeOptions = (typeField!.options as INodePropertyOptions[]).map((o) => o.value);
		...
		it('should route document number to customer.document.number', () => {
			...
			const docValues = (docField!.options as INodePropertyCollection[])[0].values;
			const numberField = docValues.find((v) => v.name === 'number');
	...
	describe('Customer resource', () => {
		...
		it('should support document in customer additional fields', () => {
			...
			const docField = (additional!.options as INodeProperties[]).find(
				(o) => o.name === 'document',
			);
			expect(docField).toBeDefined();
			const docValues = (docField!.options as INodePropertyCollection[])[0].values;
	...
	describe('Customer address fields', () => {
		...
		it('should have all address fields', () => {
			const address = findCustomerProp('addressFields');
			const fieldNames = (address!.options as INodeProperties[]).map((o) => o.name);
		...
		it('should default country to br', () => {
			const address = findCustomerProp('addressFields');
			const country = (address!.options as INodeProperties[]).find((o) => o.name === 'country');
			expect(country!.default).toBe('br');
		});

		it('should route address fields with address. prefix', () => {
			const address = findCustomerProp('addressFields');
			(address!.options as INodeProperties[]).forEach((opt) => {
