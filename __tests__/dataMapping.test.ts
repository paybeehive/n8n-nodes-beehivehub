...
function findNestedProperty(props: INodeProperties[], parentName: string, childName: string): INodeProperties | undefined {
	const parent = findProperty(props, parentName);
	if (!parent?.options) return undefined;
	for (const opt of parent.options) {
		if (opt.name === childName && 'type' in opt) return opt as INodeProperties;
		if ('values' in opt) {
			const found = (opt as INodePropertyCollection).values.find((v) => v.name === childName);
			if (found) return found;
		}
		if ('type' in opt && (opt as INodeProperties).options) {
			for (const subOpt of (opt as INodeProperties).options!) {
				if (subOpt.name === childName && 'type' in subOpt) return subOpt as INodeProperties;
				if ('values' in subOpt) {
					const found = (subOpt as INodePropertyCollection).values.find((v) => v.name === childName);
					if (found) return found;
				}
			}
		}
...
describe('Data Mapping - Monetary Values', () => {
	describe('Transaction amounts', () => {
		...
		it('should map item unit prices as cents', () => {
			...
			const itemOption = (items!.options as INodePropertyCollection[])[0];
			const unitPrice = itemOption.values.find((v) => v.name === 'unitPrice');
		...
		it('should map shipping fee as cents', () => {
			...
			const fee = (shippingFields!.options as INodeProperties[]).find((o) => o.name === 'fee');
	...
	describe('Balance routing', () => {
		it('should route balance request to GET /balance/available', () => {
			...
			const getAvailable = (operation!.options as INodePropertyOptions[]).find(
				(o) => o.value === 'getAvailable',
			);
...
describe('Data Mapping - Request Routing', () => {
	describe('Transaction operations routing', () => {
		...
		it('should route getMany to GET /transactions', () => {
			const op = getOperation();
			const getMany = (op!.options as INodePropertyOptions[]).find((o) => o.value === 'getMany');
		...
		it('should route refund to POST /transactions/{id}/refund', () => {
			const op = getOperation();
			const refund = (op!.options as INodePropertyOptions[]).find((o) => o.value === 'refund');
		...
		it('should route updateDelivery to PUT /transactions/{id}/delivery', () => {
			const op = getOperation();
			const update = (op!.options as INodePropertyOptions[]).find((o) => o.value === 'updateDelivery');
